...
for (const width of [320, 390, 768, 1024, 1440]) {
  test(`layout e navegação em ${width}px`, async ({ page }) => {
    ...
    await expect(page.locator('#depoimentos article')).toHaveCount(3);
    await expect(page.locator('#depoimentos .badge')).toHaveText(['Depoimento fictício', 'Depoimento fictício', 'Depoimento fictício']);
    ...
    for (const link of await page.locator('a[target="_blank"]').all()) {
      expect(await link.getAttribute('href')).toMatch(/^https:\/\/(www.instagram.com\/dralilianleal.otorrino\/|sandieu.com.br\/corpoclinico\/dra-lilian-leal\/|bvsms.saude.gov.br\/)/);
